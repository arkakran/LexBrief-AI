...
def init_services():
    """Initialize required services once"""
    if services:
        return
...
def analyze():
    ...
    try:
        ...
        final_points = services['post_processor'].rank_with_diversity(
            enriched_points, top_k=settings.final_output_count
        )
        ...
        logger.info("analysis_complete", duration=processing_time, points=len(final_points))
...
if __name__ == '__main__':
    ...
    logger.info("starting_flask_app", port=port)
